<?php
/**
 * Компонент поста Friendscape
 * 
 * Принимает:
 *   $post          – массив с данными поста
 *   $currentUserId – ID текущего пользователя
 *   $canCommentPost (опционально) – bool, можно ли комментировать
 */

// Защита от прямого вызова
if (!isset($post) || !isset($currentUserId)) {
    return;
}
...
// Определяем реакцию текущего пользователя, если ещё не определена
...
// Имя автора: всегда ссылка (как было раньше)
if ($isOwnPost) {
    $authorNameHtml = '<a href="profile.php">' . $fullName . '</a>';
} else {
    $authorNameHtml = '<a href="user.php?id=' . $post['user_id'] . '">' . $fullName . '</a>';
}

// Время поста (точка добавляется через CSS)
...
if (!empty($post['content'])) {
    $escaped = esc($post['content']);
    $textWithHashtags = preg_replace(
        '/#([\w\p{L}]+)/u',
        '<a href="search.php?q=%23$1" class="hashtag-link">#$1</a>',
        $escaped
    );
    $textHtml = '<div class="postBodyText">' . $textWithHashtags . '</div>';
}
...
// Комментарии: проверяем переданный флаг, по умолчанию true
$canComment = !isset($canCommentPost) || $canCommentPost;
?>
