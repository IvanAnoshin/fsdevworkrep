...
// ---------- Форматирование времени на клиенте (для свежеотправленных постов) ----------
        ...
        btn.addEventListener('click', async function() {
            ...
            try {
                ...
                if (response.success) {
                    ...
                    postDiv.querySelector('.positiveCounter').textContent = formatCount(response.likes_count);
                    postDiv.querySelector('.negativeCounter').textContent = formatCount(response.dislikes_count);
...
function renderPostHTML(post, avatarSrc, fullName) {
    ...
    const footerHtml = `<div class="postFooter">
...
kop.on('.profileNavigation__btn', 'click', function(e) {
    ...
    const container = document.getElementById('posts-container');
    const noPostsPlaceholder = container ? container.querySelector('.no-posts-placeholder') : null;
