...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            // Генерация и сохранение Friendscape Паспорта В БД (и хеш, и сырой)
            $newRawPassport = bin2hex(random_bytes(8));
            $newHashedPassport = password_hash($newRawPassport, PASSWORD_DEFAULT);
            
            $updatePassportStmt = $pdo->prepare("UPDATE users SET passport_hash = ?, passport_raw = ? WHERE id = ?");
            $updatePassportStmt->execute([$newHashedPassport, $newRawPassport, $userId]);

            $_SESSION['user_id'] = $userId;
            // НЕ сохраняем паспорт в сессии — он теперь в БД
    ...
    $_SESSION['old'] = ['first_name' => $firstName, 'last_name' => $lastName, 'gender' => $gender];
    ...
    <div class="registerPageArea">
        ...
        <div class="authLoginSection">
            <form method="post" action="register.php">
                <input type="text" name="first_name" placeholder="Имя" value="<?= htmlspecialchars($old['first_name'] ?? '') ?>" class="<?= isset($errors['first_name']) ? 'input-error' : '' ?>">
                <?php if (isset($errors['first_name'])): ?><span class="error-message"><?= $errors['first_name'] ?></span><?php endif; ?>
                <input type="text" name="last_name" placeholder="Фамилия" value="<?= htmlspecialchars($old['last_name'] ?? '') ?>" class="<?= isset($errors['last_name']) ? 'input-error' : '' ?>">
                <?php if (isset($errors['last_name'])): ?><span class="error-message"><?= $errors['last_name'] ?></span><?php endif; ?>
                ...
                <?php if (isset($errors['password2'])): ?><span class="error-message"><?= $errors['password2'] ?></span><?php endif; ?>
                <div class="sex">
                    <fieldset>
                        <legend>Ваш пол: </legend>
                        <label><input type="radio" name="gender" value="male" required <?= (isset($old['gender']) && $old['gender'] === 'male') ? 'checked': '' ?>> Мужчина</label>
                        <label><input type="radio" name="gender" value="female" <?= (isset($old['gender']) && $old['gender'] === 'female') ? 'checked' : '' ?>> Женщина</label>
                    </fieldset>
                </div>
                <?php if (isset($errors['db'])): ?><p class="error-message"><?= $errors['db'] ?></p><?php endif; ?>
